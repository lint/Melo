...
+ (DelaunayVoronoi *) voronoiWithPoints: (NSArray *) points plotBounds: (CGRect) plotBounds {
   DelaunayVoronoi *voronoi = [[DelaunayVoronoi alloc] init];
...
- (void) fortunesAlgorithm {
   ...
   while (true) {
      ...
      if ((newSite != nil) && ([heap empty] || newSiteIsLessThanNewInTStar))
      {
         ...
         [triangles addObject:[[DelaunayTriangle alloc] initWithSite1:bottomSite site2:topSite site3:[self rightRegion:lbnd]]];
